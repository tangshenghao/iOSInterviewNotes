...
Core-Animation在RunLoop中注册了一个Observer，监听了BeforeWaiting和Exit事件，当监听时间到来时，回调函数会遍历所有待处理的视图，以执行实际的绘制和调整，最后更新UI界面。如下图：

![](/Users/mikkotang/Library/Application Support/typora-user-images/image-20220728232533015.png)
...
![](/Users/mikkotang/Library/Application Support/typora-user-images/image-20220728235955048.png)

如果是卡死状态，则需要进一步处理，先设定一个卡死阀值，如果超过阀值时，获取全线程的堆栈，并保存到本地中，每个一段时间再进行一次采样，可以统计出卡死的持续时间。直到下一次重启App时，从本地取出卡死的堆栈及持续时间等信息，进行卡死上报。

![](/Users/mikkotang/Library/Application Support/typora-user-images/image-20220729000342812.png)

这种处理逻辑，好处是根据runloop状态来准确判断卡顿，并且可以抓取到堆栈。但存在一个问题是，如果超过阀值时，获取的堆栈可能并不是真实的造成卡顿的堆栈，比如下图这种情况

![](/Users/mikkotang/Library/Application Support/typora-user-images/image-20220729000922035.png)
...
![](/Users/mikkotang/Library/Application Support/typora-user-images/image-20220729003817321.png)
...
![](/Users/mikkotang/Library/Application Support/typora-user-images/image-20220729014307186.png)
...
![](/Users/mikkotang/Library/Application Support/typora-user-images/image-20220729013226521.png)
